...
class PlayerStats:
    def __init__(self, player_name):
        ...
        self.stats_df = pd.DataFrame(columns=['Hands Played', 'Wins', 'Losses', 'Folds', 'Total Bets', 'Aggressive Actions', 'Total Pot Contribution'])
        self.player_name = player_name
        # Initialize stats
        self.stats_df.loc[0] = [0, 0, 0, 0, 0, 0, 0]
...
# Game class with automated data visualization
class Game:
    ...
    def generate_data_visualizations(self):
        ...
        action_counts = game_log.groupby(['Player', 'Action']).size().unstack(fill_value=0)
        print("Action Counts per Player:")
        print(action_counts)

        # Plotting actions
        action_counts.plot(kind='bar', stacked=True, figsize=(10, 7))
        plt.title('Player Actions')
        plt.xlabel('Player')
        plt.ylabel('Number of Actions')
        plt.legend(title='Actions', bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        plt.savefig('player_actions.png')
        plt.show()

        # Clustering players based on behavior
        if len(action_counts) >= 2:
            kmeans = KMeans(n_clusters=2)
            kmeans.fit(action_counts)
            action_counts['Cluster'] = kmeans.labels_
            print("Player Clusters:")
            print(action_counts[['Cluster']])

            # Visualize clusters
            plt.figure(figsize=(8, 6))
            for cluster in action_counts['Cluster'].unique():
                cluster_data = action_counts[action_counts['Cluster'] == cluster]
                plt.scatter(cluster_data.index, cluster_data.sum(axis=1), label=f'Cluster {cluster}')
            plt.title('Player Clusters Based on Actions')
            plt.xlabel('Player')
